...
func formatSignal(signal Signal, threshold int) string {
	...
	return fmt.Sprintf("%s:%.f %s %s %.4f | %.2f | (%+v|%d) ", signal.Coin, signal.Duration.Minutes()*float64(signal.Segments), emoji.MapType(signal.Type), emoji.MapDeca(factor), signal.Price, signal.Density, signal.Duration, signal.Segments)
}
