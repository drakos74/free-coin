...
// AddTargets adds api endpoint targets for the reporting functionalities
func AddTargets(client api.Exchange, grafana *metrics.Server, registryConstr storage.EventRegistry) {
	grafana.Target("PnL", readFromRegistry(client, registryConstr, noSuffix(processor.ErrorSuffix, processor.IgnoredSuffix), addPnL))
	grafana.Target("trades", readFromRegistry(client, registryConstr, noSuffix(processor.ErrorSuffix, processor.IgnoredSuffix), count))
	grafana.Target("errors", readFromRegistry(client, registryConstr, withSuffix(processor.ErrorSuffix), count))
}
...
func parseEvents(queryGen queryGenerator) (metrics.Series, error) {
	if queryGen.condition(queryGen.dir) {
		...
		var sortedOrders processor.Orders = orders
...
func addPnL(query query) metrics.Series {
	...
	openingOrders := make(map[string]processor.Order)
