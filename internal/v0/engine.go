...
type Engine struct {
	coin       model.Coin
	uuid       string
	main       func(engineUUID string, coin model.Coin, reaction chan<- api.Signal) Processor
...
func NewEngine(c model.Coin, main func(engineUUID string, coin model.Coin, reaction chan<- api.Signal) Processor) *Engine {
	return &Engine{
		coin:       c,
...
func (engine Engine) RunWith(finished chan<- api.Signal, client api.Client) (Engine, error) {

	pipelineSignal := make(chan api.Signal)
	...
	tradeInput, err := client.Trades(pipelineSignal)
	...
	go func(reaction chan<- api.Signal, engine Engine) {
		main := engine.main(engine.uuid, engine.coin, reaction)
		defer func() {
			log.Info().Msg("engine closed")
			main.Gather()
			finished <- api.NewSignal("engine").
				ForCoin(engine.coin).
				WithID(engine.uuid).
				Create()
		}()
...
func NewOverWatch(client api.Client, user api.User) *OverWatch {
	return &OverWatch{
...
func (o *OverWatch) Start(c model.Coin, processor func(engineUUID string, coin model.Coin, reaction chan<- api.Signal) Processor, processors ...api.Processor) error {
	exec := engine{
		e:        NewEngine(c, processor),
		finished: make(chan api.Signal),
	}
	o.counter.Add(1)
...
func Log(uuid string, coin model.Coin, processed chan<- api.Signal) Processor {
