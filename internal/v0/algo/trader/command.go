...
const (
	PositionsReport     = "positions-report"
	positionsPrefix     = "?p"
	OnOffSwitch         = "signal-on-off"
	onOffPrefix         = "?r"
	Configuration       = "signal-configuration"
	configPrefix        = "?m"
	TradeInterface      = "trader-actions"
	tradePrefix         = "?t"
	NotificationCommand = "notification-status"
	notificationPrefix  = "?n"
...
// configure defines the configuration actions of the user
// prefix : ?m
// arg0 : [order-size]
...
// switchOnOff allows the user to switch the trader processor on or off.
// prefix : ?r
// arg0 : [command (start|stop)]
func (t *trader) switchOnOff(user api.User) {
	for command := range user.Listen(t.compoundKey(OnOffSwitch), onOffPrefix) {
...
// portfolio allows the user to view their portfolio.
// e.g. current open positions and balance.
// prefix : ?p
func (t *trader) portfolio(client api.Exchange, user api.User) {
	for command := range user.Listen(t.compoundKey(PositionsReport), positionsPrefix) {
...
// trade allows the user to manually trade or close positions.
// prefix : ?t
// arg0 : coin to trade to
// arg1 : action (buy,from|sell,to)
// arg2 : coin to trade from (coin|"all")
func (t *trader) trade(client api.Exchange, user api.User) {
	for command := range user.Listen(t.compoundKey(TradeInterface), tradePrefix) {
...
// notify allows the user to set their notification status.
// prefix : ?n
// TODO
func (t *trader) notify(user api.User) {
	for command := range user.Listen(t.compoundKey(NotificationCommand), notificationPrefix) {
		...
		_, err := command.Validate(
			api.AnyUser(),
			api.Contains(notificationPrefix),
			api.OneOf(&action, "start", "stop", ""),
		)
