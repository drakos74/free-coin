...
// runReportingServer starts the reporting server.
func runReportingServer(id string, port int, eventRegistry storage.EventRegistry, client api.Exchange) {
	// init grafana monitor
	grafana := metrics.NewServer(fmt.Sprintf("%s-grafana", id), port)
